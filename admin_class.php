...
// Include current month (Kenyan rent logic)
$months += 1;
		$data['months'] = $months;
		...
		// GET WATER TOTAL
$water = $this->db->query("SELECT amount FROM water_readings WHERE tenant_id = ".$id." ORDER BY billing_month DESC LIMIT 1");
...
// SEND DATA
$data['water'] = number_format($remaining_water,2);
$data['outstanding'] = number_format($outstanding,2);
	...
	if($units_used < 0) $units_used = 0;

	$amount = $units_used * $rate;

	$data = " tenant_id = '$tenant_id' ";
	$data .= ", prev_reading = '$prev_reading' ";
	$data .= ", curr_reading = '$curr_reading' ";
	$data .= ", units_used = '$units_used' ";
	$data .= ", rate = '$rate' ";
	$data .= ", amount = '$amount' ";
	$data .= ", billing_month = '$billing_month' ";

	$save = $this->db->query("INSERT INTO water_readings set $data");
...
function get_tenant_house(){
	extract($_POST);

	$qry = $this->db->query("SELECT h.house_no FROM tenants t INNER JOIN houses h ON h.id = t.house_id WHERE t.id = $id");

	if($qry->num_rows > 0){
		return $qry->fetch_array()['house_no'];
	}
}
function get_house_tenant(){
    extract($_POST);

    $qry = $this->db->query("
        SELECT t.id, concat(t.lastname,', ',t.firstname,' ',t.middlename) as name 
        FROM tenants t 
        WHERE t.house_id = $id AND t.status = 1
    ");
...
function delete_water(){
    extract($_POST);

    $delete = $this->db->query("DELETE FROM water_readings WHERE id = $id");

    if($delete)
        return 1;
}

function get_last_reading(){
    extract($_POST);

    $qry = $this->db->query("SELECT curr_reading FROM water_readings WHERE tenant_id = $id ORDER BY id DESC LIMIT 1");

    if($qry && $qry->num_rows > 0){
        echo $qry->fetch_array()['curr_reading'];
    }else{
        echo '';
    }
}
