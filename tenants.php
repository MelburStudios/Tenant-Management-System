						...
						<table class="table table-condensed table-bordered table-hover">
							<thead>
								<tr>
									...
									<th class="">Water Balance</th>
									<th class="">Total Outstanding Balance</th>
							...
							<tbody>
								<?php 
								$i = 1;
								$tenant = $conn->query("SELECT t.*,concat(t.lastname,', ',t.firstname,' ',t.middlename) as name,h.house_no,h.price FROM tenants t inner join houses h on h.id = t.house_id where t.status = 1 ORDER BY t.id ASC");
								while($row=$tenant->fetch_assoc()):
									$months = abs(strtotime(date('Y-m-d')." 23:59:59") - strtotime($row['date_in']." 23:59:59"));
									$months = ceil(($months) / (30*60*60*24));
...
// 🔥 GET WATER
$water = $conn->query("SELECT amount FROM water_readings WHERE tenant_id = ".$row['id']." ORDER BY billing_month DESC LIMIT 1");
$water_total = $water->num_rows > 0 ? $water->fetch_array()['amount'] : 0;

// 🔥 FINAL OUTSTANDING
$outstanding = ($payable + $water_total) - $paid;
								?>
								<tr>
...
<td class="text-right">
     <p><b><?php echo number_format($water_total,2) ?></b></p>
</td>
